...
class Softmaxer(nn.Module):
    def __init__(self, num_inputs=784, num_outputs=62, lr=.01):
        super().__init__()
        self.num_inputs=num_inputs
        self.num_outputs=num_outputs
        self.lr=lr

        self.net = nn.Sequential(nn.Flatten(), nn.LazyLinear(num_outputs)) #Flattens input layer then creates a linear connection between inputs and outputs.
